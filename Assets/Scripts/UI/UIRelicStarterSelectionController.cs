...
public class UIRelicStarterSelectionController : Singleton<UIRelicStarterSelectionController>
{
    ...
    void Start()
    {
        m_starterRelics[0].Init(new ContentLivingStoneRelic(), UIRelic.RelicSelectionType.SelectStarter);
        m_starterRelics[1].Init(new ContentMaskOfAgesRelic(), UIRelic.RelicSelectionType.SelectStarter);
